...
const ReportsView: React.FC<ReportsViewProps> = ({ vacancies, convocations }) => {
  ...
  const exportCSV = () => {
    const headers = ["Edital", "Unidade", "Candidato", "Nº Vaga/Posto", "Ciclo Ocupação", "Início Contrato", "Expectativa de Fim", "Projeção Máxima Vaga", "Dias Restantes Contratado", "Situação"];
          ...
          <table className="w-full text-left text-[11px] whitespace-nowrap">
            <thead className="bg-slate-50 text-[10px] uppercase font-black text-slate-400 border-b border-slate-100">
              <tr>
                ...
                <th className="px-6 py-5 text-center">Saldo Contratado</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {reportData.length > 0 ? reportData.map((d, i) => (
                <tr key={i} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-5 text-center">
                    <div className="flex flex-col">
                        ...
                        <div className="w-12 h-1 bg-slate-100 mx-auto rounded-full mt-1">
                ...
                <tr><td colSpan={6} className="px-6 py-20 text-center text-slate-300 italic font-bold uppercase text-[10px]">Nenhum dado encontrado para auditoria.</td></tr>
              )}
            </tbody>
          </table>
        </div>
