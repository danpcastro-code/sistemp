...
import { 
  Search, Plus, ChevronRight, Building2, Clock, FastForward, Trash2, MapPin, X, FilterX
} from 'lucide-react';
...
const VacancyManagement: React.FC<VacancyManagementProps> = ({ vacancies, setVacancies, parameters, agencies, units, profiles, setAgencies, setUnits, convocations, setConvocations, userRole, onLog }) => {
  ...
  // Funções de gatilho para modais de contrato
  ...
  return (
    <div className="space-y-6">
      {!selectedVacancy ? (
        ...
        <div className="space-y-8 animate-in fade-in duration-300">
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-4">
                ...
                {selectedSlotFilter !== null && (
                  <button onClick={() => setSelectedSlotFilter(null)} className="px-4 py-1.5 bg-red-50 text-red-600 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-red-100 transition-all flex items-center shadow-sm">
                      <FilterX size={14} className="mr-2"/> Excluir Seleção (Ver Todos do Grupo)
                  </button>
                )}
            </div>
          ...
          <div className="bg-white p-8 rounded-[3rem] border border-slate-200 shadow-xl shadow-slate-200/50 relative">
                  ...
                  return (
                    <button 
                      ...
                      className={`w-7 h-7 rounded-md flex items-center justify-center text-[8px] font-black border transition-all ${
                          isSelected ? 'ring-4 ring-blue-500/30 border-blue-600 scale-110 z-10' : 'hover:scale-105'
                      } ${
                          activeOcc ? 'bg-green-500 border-green-600 text-white shadow-[0_0_8px_#22c55e]' : 
            ...
            <div className="mt-12 overflow-x-auto rounded-3xl border border-slate-100 shadow-sm bg-white">
                <table className="w-full text-left">
                                        ...
                                        {isActive && canEdit && (
                                            <div className="flex justify-end space-x-2">
                                                <button 
                                                  ...
                                                  className="p-2 border border-slate-200 rounded-lg hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-all text-slate-400 shadow-sm" 
                                                  title="Prorrogar"
                                                >
                                                  <FastForward size={14} />
                                                </button>
                                                <button 
                                                  ...
                                                  className="p-2 border border-slate-200 rounded-lg hover:bg-red-500 hover:text-white hover:border-red-500 transition-all text-slate-400 shadow-sm" 
                                                  title="Rescindir"
                                                >
                                                  <Trash2 size={14} />
                                                </button>
                        ...
                        {displayedOccupations.length === 0 && (
                          <tr>
                            <td colSpan={6} className="px-6 py-10 text-center text-slate-400 font-medium italic">
                               Nenhum contrato ativo neste filtro.
                            </td>
                          </tr>
                        )}
                    </tbody>
      ...
      {showExtendModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[999] flex items-center justify-center p-4 overflow-hidden">
          <div className="bg-white rounded-[2rem] max-w-sm w-full p-8 shadow-2xl animate-in zoom-in duration-200 border border-slate-100 relative">
            <button onClick={() => setShowExtendModal(false)} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600"><X size={20}/></button>
            ...
            <form onSubmit={handleExtendContract} className="space-y-4">
              <div>
                ...
                <input value={extAmendmentTerm} onChange={e => setExtAmendmentTerm(e.target.value)} required className="mt-2 w-full border border-slate-200 rounded-xl p-3 text-sm outline-none font-bold focus:ring-2 focus:ring-blue-500/20" placeholder="Ex: TA 01/2024"/>
              </div>
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Nova Data Final</label>
                <input type="date" value={extNewEndDate} onChange={e => setExtNewEndDate(e.target.value)} required className="mt-2 w-full border border-slate-200 rounded-xl p-3 text-sm outline-none font-bold focus:ring-2 focus:ring-blue-500/20"/>
              </div>
              <div className="flex justify-end gap-3 mt-6">
                ...
                <button type="submit" className="px-8 py-3 bg-blue-600 text-white font-black text-[10px] uppercase tracking-widest rounded-xl shadow-lg active:scale-95 transition-all">Confirmar</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showRescindModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[999] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2rem] max-w-sm w-full p-8 shadow-2xl animate-in zoom-in duration-200 border-2 border-red-50 relative">
            <button onClick={() => setShowRescindModal(false)} className="absolute top-4 right-4 text-slate-400 hover:text-red-600"><X size={20}/></button>
            ...
            <form onSubmit={handleRescindContract} className="space-y-4">
              <div>
                ...
                <input type="date" value={rescindDate} onChange={e => setRescindDate(e.target.value)} required className="mt-2 w-full border border-slate-200 rounded-xl p-3 text-sm outline-none font-bold focus:ring-2 focus:ring-red-500/20"/>
              </div>
              <div className="flex justify-end gap-3 mt-6">
                ...
                <button type="submit" className="px-8 py-3 bg-red-600 text-white font-black text-[10px] uppercase tracking-widest rounded-xl shadow-lg active:scale-95 transition-all">Confirmar Rescisão</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showAddModal && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[999] flex items-center justify-center p-4">
             <div className="bg-white rounded-[2.5rem] max-w-lg w-full p-10 shadow-2xl animate-in zoom-in duration-200 border border-slate-100 relative">
      ...
      {showAddContractModal && targetSlotInfo && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[999] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] max-w-md w-full p-10 shadow-2xl animate-in zoom-in duration-200 border border-slate-100 relative">
            <button onClick={() => setShowAddContractModal(false)} className="absolute top-6 right-6 text-slate-400 hover:text-slate-600"><X size={20}/></button>
