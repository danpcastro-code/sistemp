...
const VacancyManagement: React.FC<VacancyManagementProps> = ({ vacancies, setVacancies, parameters, agencies, units, profiles, setAgencies, setUnits, convocations, setConvocations, pssList, userRole, onLog }) => {
  ...
  const [editingOcc, setEditingOcc] = useState<Occupation | null>(null);
  const [adminEditStartDate, setAdminEditStartDate] = useState('');
  const [adminEditEndDate, setAdminEditEndDate] = useState('');
  const [adminEditTerm, setAdminEditTerm] = useState('');
  ...
  // Identifica candidatos que pertencem ao PSS mas NÃO foram convocados ainda (para ajuda ao usuário)
  const candidatesNotConvokedYet = useMemo(() => {
    if (!selectedVacancy) return [];
    const vacancyPssId = String(selectedVacancy.pssId || '').trim();
    return convocations.filter(c => 
      String(c.pssId || '').trim() === vacancyPssId && 
      (!c.convocationAct || c.convocationAct.trim() === "")
    );
  }, [convocations, selectedVacancy]);
  ...
  const handleSaveVacancy = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    onLog('CRIAR_VAGA', `Grupo ${newVacancy.code} criado e vinculado ao PSS ID ${pssId}.`);
  };
  ...
  const triggerAdminEdit = (occ: Occupation) => {
    setEditingOcc(occ);
    setAdminEditStartDate(occ.startDate);
    setAdminEditEndDate(occ.endDate);
    setAdminEditTerm(occ.amendmentTerm || '');
    setAdminEditReason(occ.terminationReason || '');
    setShowAdminEditModal(true);
  };

  const handleAdminSaveOcc = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedVacancy || !editingOcc) return;

    setVacancies(prev => prev.map(v => v.id === selectedVacancy.id ? {
      ...v,
      occupations: v.occupations.map(o => o.id === editingOcc.id ? {
        ...o,
        startDate: adminEditStartDate,
        endDate: adminEditEndDate,
        amendmentTerm: adminEditTerm.toUpperCase(),
        terminationReason: adminEditReason,
        isExtensionRequired: adminEditEndDate < o.projectedFinalDate
      } : o)
    } : v));

    onLog('EDICAO_ADMIN', `Administrador corrigiu dados do contrato de ${editingOcc.contractedName}.`);
  ...
  return (
    <div className="space-y-6">
                ...
                {filteredVacancies.map(v => (
                  <tr key={v.id} className="hover:bg-slate-50 transition-colors group">
                    <td className="px-8 py-5">
                      ...
                      <p className="text-[10px] text-slate-400 font-bold uppercase">{v.type}</p>
                      <span className="text-[8px] bg-slate-100 px-1.5 py-0.5 rounded-sm font-black text-slate-500 uppercase mt-1 inline-block">PSS: {pssList.find(p => p.id === v.pssId)?.title || 'Vínculo Ausente'}</span>
                    </td>
            ...
            <div className="mb-8">
                ...
                {selectedVacancy.pssId && (
                  <div className="mt-5 flex items-center space-x-2 bg-slate-50 px-4 py-2 rounded-xl border border-slate-100 w-fit">
                    ...
                    <span className="text-[9px] font-black uppercase text-slate-500 tracking-widest">PSS: {pssList.find(p => p.id === selectedVacancy.pssId)?.title || 'Vínculo Indisponível'}</span>
                  </div>
                )}
            </div>
            ...
            <div className="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100 mb-10">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="flex items-center space-x-4">
                        <div className="flex items-center space-x-1.5">
                            ...
                            <span className="text-[8px] font-black text-slate-400 uppercase">Saldo Parcial (Degradado)</span>
                        </div>
                        {selectedSlotFilter !== null && (
                            <button onClick={() => setSelectedSlotFilter(null)} className="text-[9px] font-black text-blue-600 uppercase flex items-center hover:underline">
                                <FilterX size={12} className="mr-1"/> Limpar Filtro
                            </button>
                        )}
                    </div>
                </div>
                <div className="flex flex-wrap gap-3">
                  {Array.from({ length: selectedVacancy.initialQuantity }).map((_, i) => {
                    ...
                    const isPartiallyConsumed = remDays > 0 && remDays < selectedVacancy.maxTermDays;

                    return (
                      <button 
                        key={idx} 
                        onClick={() => handleSlotClick(idx)} 
                        title={isPartiallyConsumed ? `Vaga Degradada: Posto livre mas com saldo reduzido (${remDays} dias restantes)` : activeOcc ? `Ocupado por ${activeOcc.contractedName}` : `Posto Limpo e Habilitado`}
                        className={`w-12 h-12 rounded-2xl border-2 flex flex-col items-center justify-center font-black transition-all transform active:scale-90 ${
                            isSelected ? 'ring-4 ring-blue-500/20 border-blue-600 z-10 scale-110' : ''
                        } ${
                            activeOcc ? (isSelected ? 'bg-blue-50 border-blue-600' : 'bg-green-600 text-white border-green-700 shadow-lg') : 
                            remDays <= 0 ? 'bg-red-50 text-red-300 border-red-100 opacity-50 cursor-not-allowed' : 
                            isPartiallyConsumed ? 'bg-red-100 text-red-600 border-red-200 hover:bg-red-200 shadow-inner' :
                  ...
                  <tr key={occ.id} className={`hover:bg-slate-50 transition-colors ${occ.status === ContractStatus.ENDED ? 'opacity-40 grayscale' : ''}`}>
                    ...
                    <td className="px-8 py-5 text-right">
                      <div className="flex justify-end space-x-2">
                        {isAdmin && (
                           <button onClick={() => triggerAdminEdit(occ)} className="p-2.5 bg-white border border-slate-200 rounded-xl hover:bg-slate-900 hover:text-white transition-all shadow-sm active:scale-90" title="Edição Administrativa"><Pencil size={14}/></button>
                        )}
      ...
      {showAdminEditModal && editingOcc && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[2000] flex items-center justify-center p-4">
          <div className="bg-white rounded-[3rem] max-w-sm w-full p-10 shadow-2xl animate-in zoom-in duration-200 border border-slate-100 relative">
             <button onClick={() => setShowAdminEditModal(false)} className="absolute top-8 right-8 text-slate-400 hover:text-slate-600"><X size={20}/></button>
             <h2 className="text-2xl font-black mb-1 text-slate-800 uppercase tracking-tighter">Correção de Dados</h2>
             <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mb-8">Administrador: {editingOcc.contractedName}</p>
             <form onSubmit={handleAdminSaveOcc} className="space-y-4">
               <div className="grid grid-cols-2 gap-3">
                 <div>
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Início Contrato</label>
                    <input type="date" value={adminEditStartDate} onChange={e => setAdminEditStartDate(e.target.value)} required className="w-full border border-slate-200 rounded-2xl p-4 text-xs font-bold bg-slate-50 outline-none" />
                 </div>
                 <div>
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Fim Vigência</label>
                    <input type="date" value={adminEditEndDate} onChange={e => setAdminEditEndDate(e.target.value)} required className="w-full border border-slate-200 rounded-2xl p-4 text-xs font-bold bg-slate-50 outline-none" />
                 </div>
               </div>
               <div>
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Termo Aditivo / Ato</label>
                  <input value={adminEditTerm} onChange={e => setAdminEditTerm(e.target.value)} className="w-full border border-slate-200 rounded-2xl p-4 text-xs font-bold bg-slate-50 outline-none" placeholder="Ex: TA 01/2024" />
               </div>
               {editingOcc.status === ContractStatus.ENDED && (
                 <div>
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Motivo Encerramento</label>
                    <input value={adminEditReason} onChange={e => setAdminEditReason(e.target.value)} className="w-full border border-slate-200 rounded-2xl p-4 text-xs font-bold bg-slate-50 outline-none" />
                 </div>
               )}
               <button type="submit" className="w-full py-4 bg-slate-900 text-white font-black text-[10px] uppercase rounded-2xl shadow-xl mt-4">Salvar Alterações</button>
             </form>
          </div>
        </div>
      )}
      ...
      {showAddContractModal && targetSlotInfo && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[2000] flex items-center justify-center p-4">
          <div className="bg-white rounded-[3rem] max-w-lg w-full p-12 shadow-2xl animate-in zoom-in duration-200 border border-slate-100 relative max-h-[90vh] overflow-y-auto custom-scrollbar">
            ...
            <h2 className="text-3xl font-black mb-8 text-slate-800 uppercase tracking-tighter leading-tight">Provimento Posto #{targetSlotInfo.slotIndex}</h2>
            
            <form onSubmit={handleAddContract} className="space-y-6">
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Selecionar Candidato Habilitado</label>
                <select value={selectedPersonId} onChange={e => setSelectedPersonId(e.target.value)} required className="mt-2 w-full border border-slate-200 rounded-2xl p-4 text-sm font-bold bg-slate-50 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all">
                ...
                {sortedPendingCandidates.length === 0 && (
                   <div className="mt-6 p-6 bg-red-50 rounded-3xl border border-red-100 flex flex-col space-y-4 shadow-inner">
                      <div className="flex items-start space-x-3">
                        <UserX className="text-red-500 mt-1 shrink-0" size={18} />
                        <div>
                          <p className="text-[10px] text-red-600 font-black uppercase leading-tight mb-1">Nenhum Candidato Habilitado Identificado</p>
                          <p className="text-[9px] text-red-400 font-bold leading-relaxed uppercase">O sistema aplica filtros estritos baseados na parametrização do grupo:</p>
                        </div>
                      </div>
                      
                      <div className="grid grid-cols-1 gap-2 pl-7">
                        <div className="flex items-center space-x-2">
                           <CheckCircle size={10} className={selectedVacancy?.pssId ? "text-green-500" : "text-red-400"} />
                           <span className="text-[9px] font-bold text-slate-500 uppercase">Edital (ID): {pssList.find(p => p.id === selectedVacancy?.pssId)?.title || 'Vínculo Inválido'}</span>
                        </div>
                        <div className="flex items-center space-x-2">
                           <Briefcase size={10} className="text-blue-500" />
                           <span className="text-[9px] font-bold text-slate-500 uppercase">Perfil Requerido: {selectedVacancy?.type}</span>
                        </div>
                        <div className="flex items-center space-x-2">
                           <Clock size={10} className="text-amber-500" />
                           <span className="text-[9px] font-bold text-slate-500 uppercase">Status: Deve ter "Chamamento Realizado" (Convocado)</span>
                        </div>
                      </div>

                      {candidatesNotConvokedYet.length > 0 && (
                        <div className="mt-2 pt-4 border-t border-red-100">
                           <p className="text-[8px] font-black text-red-500 uppercase flex items-center">
                             <HelpCircle size={10} className="mr-1" /> Foram encontrados {candidatesNotConvokedYet.length} candidatos neste edital, mas eles ainda não foram convocados.
                           </p>
                           <p className="text-[8px] text-slate-400 font-bold mt-1 uppercase">Ação: Vá ao menu "Classificados PSS" e realize o chamamento desses candidatos.</p>
                        </div>
                      )}
                   </div>
                )}
              ...
              <div className="flex justify-end gap-4 mt-10">
                ...
                <button type="submit" disabled={!selectedPersonId} className="px-10 py-4 bg-blue-600 text-white font-black text-xs uppercase rounded-2xl shadow-xl active:scale-95 disabled:opacity-50 transition-all">Confirmar Contratação</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[2000] flex items-center justify-center p-4">
          <div className="bg-white rounded-[3rem] max-w-md w-full p-12 shadow-2xl relative animate-in zoom-in duration-200 border border-slate-100">
            ...
            <form onSubmit={handleSaveVacancy} className="space-y-6">
              ...
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">PSS de Origem (Vínculo Obrigatório)</label>
