        ...
        <div className="space-y-8 animate-in fade-in duration-300">
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-4">
                <button onClick={() => { setSelectedVacancyId(null); setSelectedSlotFilter(null); }} className="text-blue-600 font-bold flex items-center text-sm hover:translate-x-[-4px] transition-transform group">
                    <ChevronRight size={16} className="rotate-180 mr-1 group-hover:mr-2 transition-all"/> Voltar para Lista
                </button>
                {selectedSlotFilter !== null && (
                  <button onClick={() => setSelectedSlotFilter(null)} className="px-4 py-1.5 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-800 transition-all flex items-center shadow-lg border border-white/10">
                      <FilterX size={14} className="mr-2"/> Limpar Filtro (Ver Tudo)
                  </button>
                )}
            </div>
          ...
          <div className="bg-white p-10 rounded-[3.5rem] border border-slate-200 shadow-2xl shadow-slate-200/50 relative">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
                <div className="flex items-center space-x-6">
                    <div className="w-16 h-16 bg-slate-900 rounded-[1.5rem] flex items-center justify-center text-white shadow-2xl"><Building2 size={32} /></div>
                    <div>
                        <h2 className="text-4xl font-black text-slate-800 tracking-tighter">{selectedVacancy.code}</h2>
                        <div className="flex items-center space-x-4 mt-1.5">
                            <p className="text-slate-400 font-bold uppercase text-[10px] tracking-[0.2em]">{selectedVacancy.agency}</p>
                            <span className="text-slate-200">•</span>
                            <div className="flex items-center text-indigo-500 font-black uppercase text-[10px] tracking-[0.2em] bg-indigo-50 px-3 py-1 rounded-lg">
                                <MapPin size={12} className="mr-2" /> {selectedVacancy.unit}
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100 mb-10">
              <div className="flex items-center space-x-3 mb-6">
                  ...
                  <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-500">Visualização de Vagas / Postos</h3>
              </div>
              <div className="flex flex-wrap gap-2 max-h-[400px] overflow-y-auto custom-scrollbar p-1">
                {Array.from({ length: selectedVacancy.initialQuantity }).map((_, i) => {
                  ...
                  return (
                    <button 
                      ...
                      className={`w-10 h-10 rounded-xl flex flex-col items-center justify-center transition-all border shadow-sm ${
                          isSelected ? 'ring-4 ring-blue-500/30 border-blue-600 scale-110 z-10' : 'hover:scale-105 active:scale-95'
                      } ${
                          activeOcc ? 'bg-green-500 border-green-600 text-white shadow-[0_0_12px_#22c55e]' : 
                          isExhausted ? 'bg-red-100 border-red-200 text-red-300 cursor-not-allowed' :
                          slotOccupations.length > 0 ? 'bg-blue-100 border-blue-200 text-blue-600 font-bold' :
                          'bg-white border-slate-200 text-slate-400 font-medium hover:border-slate-400'
                      }`}
                    >
                      <span className="text-[11px] font-black">{idx}</span>
                    </button>
                  );
                })}
              </div>
            ...
            <div className="mt-8 overflow-x-auto rounded-[2rem] border border-slate-100 shadow-sm bg-white">
                <table className="w-full text-left">
                                ...
                                <tr key={occ.id} className="hover:bg-slate-50 transition-colors group">
                                    <td className="px-8 py-6">
                                        <div className="flex items-center">
                                            <span className="w-8 h-8 rounded-lg bg-slate-900 text-white flex items-center justify-center font-black mr-3 text-[10px]">#{occ.slotIndex}</span>
                                            <div>
                                                <span className="block text-[9px] font-black uppercase text-blue-600">{occ.order}ª OCUPAÇÃO</span>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-8 py-6">
                                        <span className="font-black text-slate-800 text-[12px]">{occ.contractedName}</span>
                                        <span className="block text-[9px] font-bold uppercase text-slate-400 mt-1 tracking-wider">{occ.competition}</span>
                                    </td>
                                    <td className="px-8 py-6">
                                        <div className="flex items-center text-slate-500 space-x-2">
                                            ...
                                            <span className="font-black text-slate-800">{formatDisplayDate(occ.endDate)}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-8 py-6 text-center">
                                        <div className="flex flex-col items-center">
                                            <span className={`font-black text-[13px] ${remDaysContract <= 30 ? 'text-red-500' : 'text-slate-800'}`}>
                                                {remDaysContract} dias
                                            </span>
                                            <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest mt-0.5">No Contrato</span>
                                        </div>
                                    </td>
                                    <td className="px-8 py-6 text-right uppercase text-[9px] font-black">
                                        <span className={`px-2.5 py-1 rounded-lg border ${isActive ? 'bg-green-50 text-green-600 border-green-100' : 'bg-slate-50 text-slate-400 border-slate-100'}`}>
                                            {occ.status}
                                        </span>
                                    </td>
                                        ...
                                        {isActive && canEdit && (
                                            <div className="flex justify-end space-x-2">
                                                <button 
                                                  ...
                                                  className="p-2.5 border border-slate-200 rounded-xl hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-all text-slate-400 shadow-sm active:scale-90" 
                                                  title="Prorrogar"
                                                >
                                                  <FastForward size={14} />
                                                </button>
                                                <button 
                                                  ...
                                                  className="p-2.5 border border-slate-200 rounded-xl hover:bg-red-500 hover:text-white hover:border-red-500 transition-all text-slate-400 shadow-sm active:scale-90" 
                                                  title="Rescindir"
                                                >
                                                  <Trash2 size={14} />
                                                </button>
                          ...
                          <tr>
                            <td colSpan={7} className="px-8 py-20 text-center">
                               <div className="flex flex-col items-center justify-center opacity-30">
                                   ...
                                   <p className="font-black uppercase text-[10px] tracking-widest">Nenhum contrato ativo neste filtro</p>
                               </div>
                            </td>
                          </tr>
                        )}
      ...
      {showExtendModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[1000] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] max-w-sm w-full p-10 shadow-2xl animate-in zoom-in duration-200 border border-slate-100 relative">
            <button onClick={() => setShowExtendModal(false)} className="absolute top-6 right-6 text-slate-400 hover:text-slate-600"><X size={20}/></button>
            <h2 className="text-2xl font-black mb-8 text-slate-800 uppercase tracking-tighter">Prorrogar</h2>
            <form onSubmit={handleExtendContract} className="space-y-6">
              <div>
                ...
                <input value={extAmendmentTerm} onChange={e => setExtAmendmentTerm(e.target.value)} required className="mt-2 w-full border border-slate-200 rounded-2xl p-4 text-sm outline-none font-bold focus:ring-4 focus:ring-blue-500/10 transition-all" placeholder="Ex: TA 01/2024"/>
              </div>
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Nova Data Final</label>
                <input type="date" value={extNewEndDate} onChange={e => setExtNewEndDate(e.target.value)} required className="mt-2 w-full border border-slate-200 rounded-2xl p-4 text-sm outline-none font-bold focus:ring-4 focus:ring-blue-500/10 transition-all"/>
              </div>
              <div className="flex justify-end gap-3 mt-8">
                <button type="button" onClick={() => setShowExtendModal(false)} className="px-6 py-4 font-bold text-slate-400 text-[10px] uppercase tracking-widest">Cancelar</button>
                <button type="submit" className="px-10 py-4 bg-blue-600 text-white font-black text-[10px] uppercase tracking-widest rounded-2xl shadow-xl active:scale-95 transition-all">Confirmar</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[1000] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] max-w-sm w-full p-10 shadow-2xl animate-in zoom-in duration-200 border-2 border-red-50 relative">
            ...
            <form onSubmit={handleRescindContract} className="space-y-6">
              ...
              <div className="flex justify-end gap-3 mt-8">
                <button type="button" onClick={() => setShowRescindModal(false)} className="px-6 py-4 font-bold text-slate-400 text-[10px] uppercase tracking-widest">Cancelar</button>
                <button type="submit" className="px-10 py-4 bg-red-600 text-white font-black text-[10px] uppercase tracking-widest rounded-2xl shadow-xl active:scale-95 transition-all">Confirmar</button>
              </div>
            </form>
          </div>
        </div>
             ...
             <div className="bg-white rounded-[3rem] max-w-lg w-full p-12 shadow-2xl animate-in zoom-in duration-200 border border-slate-100 relative">
                ...
                <form onSubmit={handleSaveVacancy} className="space-y-8">
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="col-span-2">
                             ...
                             <select name="unit" required className="mt-2 w-full border border-slate-200 rounded-2xl p-4 text-sm bg-slate-50 outline-none font-bold">
                                {units.map((u, i) => <option key={u} value={u}>{u}</option>)}
                             </select>
                        </div>
                        <div>
                             <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Código Identificador</label>
                             <input name="code" required className="mt-2 w-full border border-slate-200 rounded-2xl p-4 text-sm outline-none font-bold" placeholder="VAG-2024-01"/>
                        </div>
                        <div>
                   ...
                   <div className="flex justify-end gap-5 mt-10">
                      <button type="button" onClick={() => setShowAddModal(false)} className="px-10 py-4 font-bold text-slate-400 uppercase text-[11px] tracking-widest">Cancelar</button>
                      <button type="submit" className="px-14 py-4 bg-slate-900 text-white rounded-[1.5rem] font-black uppercase text-[11px] tracking-widest shadow-2xl active:scale-95 transition-all">Criar Grupo</button>
                   </div>
                </form>
             </div>
          </div>
            ...
            <form onSubmit={handleAddContract} className="space-y-8">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <input type="date" value={formStartDate} onChange={e => setFormStartDate(e.target.value)} required className="mt-2 w-full border border-slate-200 rounded-2xl p-4 text-sm outline-none font-bold focus:ring-4 focus:ring-blue-500/10"/>
                </div>
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Data Fim Prevista</label>
                  <input type="date" value={formEndDate} onChange={e => setFormEndDate(e.target.value)} required className="mt-2 w-full border border-slate-200 rounded-2xl p-4 text-sm outline-none font-bold focus:ring-4 focus:ring-blue-500/10"/>
                </div>
              </div>
              <div className="flex justify-end gap-5 mt-10">
