  ...
  return (
    <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm flex flex-col h-[450px]">
      ...
      <div className="flex space-x-2 mb-6">
        <input value={inputValue} onChange={(e) => setInputValue(e.target.value)} placeholder="Novo..." className="flex-1 bg-slate-50 border border-slate-100 rounded-xl px-4 py-2 text-xs font-bold outline-none focus:border-blue-500 transition-all" onKeyDown={(e) => e.key === 'Enter' && handleAdd()}/>
        ...
        {(items || []).map((item) => (
          <div key={item.id} className={`flex items-center justify-between p-3 rounded-xl border transition-all ${item.status === 'inactive' ? 'bg-slate-50 opacity-60 border-transparent' : 'bg-white border-slate-100'}`}>
            ...
            <div className="flex items-center">
              <button onClick={() => onToggleStatus(item)} className={`p-1.5 rounded-lg transition-colors mr-1 ${item.status === 'inactive' ? 'text-slate-300 hover:text-blue-500' : 'text-blue-500 hover:bg-blue-50'}`}>{item.status === 'inactive' ? <EyeOff size={14}/> : <Eye size={14}/>}</button>
              <button onClick={() => onAction(item)} className="p-1.5 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors"><Trash2 size={14}/></button>
            </div>
          </div>
        ))}
      </div>
...
const SettingsView: React.FC<SettingsViewProps> = ({ 
  ...
  const [newParam, setNewParam] = useState<Partial<LegalParameter>>({ label: '', days: 0, type: 'administrative', lawRef: '', articleRef: '', legalText: '', status: 'active' });
  const [newUser, setNewUser] = useState({ name: '', username: '', password: '', role: UserRole.HR });

  const REPAIR_SQL = `-- SCRIPT DE REPARO INTEGRAL SUPABASE - CTU GESTÃO v2.8
-- 1. Criação da Tabela com suporte a todas as colunas JSONB
...
-- 2. Garantia de RLS desabilitado para sincronia transparente
...
-- 3. Inserção do registro mestre
INSERT INTO public.sistemp_data (id) VALUES (1) ON CONFLICT (id) DO NOTHING;

COMMENT ON TABLE public.sistemp_data IS 'Base de dados centralizada v2.8';`;
  ...
  const handleSaveParam = (e: React.FormEvent) => {
    e.preventDefault();
    const p: LegalParameter = { id: generateId(), label: removeAccents(newParam.label || '').toUpperCase(), days: newParam.days || 0, description: '', type: 'legal', status: 'active' };
  ...
  // Fix: Added missing 'sender' property to safeEmailConfig fallback object.
  const safeEmailConfig = emailConfig || { serviceId: '', templateId: '', publicKey: '', sender: '', subject: '', template: '' };
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      {activeSubTab === 'cloud' && (
        <div className="space-y-6 animate-in fade-in">
            {/* BLOCO DE RESET - PRIORIDADE TOTAL */}
            <div className="bg-red-50 p-8 rounded-[2.5rem] border-2 border-red-500 shadow-xl">
                <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                    <div className="flex items-center space-x-5">
                        <div className="p-5 bg-red-600 text-white rounded-[1.5rem] shadow-lg"><Bomb size={32}/></div>
                        <div>
                            <h3 className="text-xl font-black text-red-800 uppercase tracking-tighter">Zona de Perigo: Master Reset</h3>
                            <p className="text-[10px] text-red-600 font-bold uppercase mt-1 tracking-widest leading-relaxed max-w-sm">Esta ação apaga permanentemente todos os registros e restaura o sistema para o padrão de fábrica.</p>
                        </div>
                    </div>
                    <button onClick={() => {
                        const pwd = window.prompt("⚠️ ATENÇÃO: ESTA AÇÃO É IRREVERSÍVEL!\nDigite a SENHA MESTRA para confirmar a exclusão de TODOS os dados:");
                        if (pwd === "1!Leinad") onRestoreAll();
                        else if (pwd !== null) alert("Senha incorreta.");
                    }} className="px-10 py-5 bg-red-600 text-white rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] shadow-2xl hover:bg-red-700 transition-all border-b-4 border-red-800 active:scale-95">Zerar Todo o Sistema</button>
                </div>
            </div>

            <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm">
                <h3 className="text-sm font-black text-slate-800 uppercase tracking-tighter flex items-center mb-6"><Activity className="mr-2 text-indigo-600" size={18}/> Estado da Conexão</h3>
                {cloudStatus === 'connected' ? (
                   <div className="p-8 bg-green-50 rounded-[2rem] border border-green-200 flex items-center justify-between">
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          <p className="text-xs font-black text-slate-800 uppercase">Sincronização OK</p>
                          <p className="text-[10px] font-bold text-green-600 uppercase tracking-widest mt-1">DADOS MEMORIZADOS NO SUPABASE</p>
                        </div>
                      </div>
                   </div>
                ) : (
                  <div className="p-8 bg-amber-50 rounded-[2rem] border border-amber-200 flex items-center justify-between animate-pulse">
                      <div className="flex items-center space-x-4">
                        ...
                        <div><p className="text-xs font-black text-slate-800 uppercase tracking-widest">Aguardando Resposta da Nuvem...</p></div>
                      </div>
                  </div>
                )}
            </div>

            <div className="bg-white p-10 rounded-[2.5rem] border-2 border-indigo-100 shadow-sm">
                <div className="flex items-center space-x-3 mb-4">
                    <DatabaseZap className="text-indigo-600" size={28}/><h3 className="text-xl font-black text-slate-800 uppercase tracking-tighter">Script SQL de Reparo v2.8</h3>
                </div>
                <div className="bg-slate-900 rounded-[1.5rem] p-6 relative border border-slate-800">
          ...
          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm">
            <div className="flex justify-between items-center mb-6">
              <div className="flex items-center">
                ...
                <div><h3 className="text-sm font-black text-slate-800 uppercase tracking-tighter">Prazos de Vigência</h3><p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">Base de cálculo legal</p></div>
              </div>
              ...
              {(parameters || []).map((p) => (
                <div key={p.id} className="p-4 rounded-2xl border border-slate-100 bg-white hover:border-blue-200 transition-all flex items-center justify-between group">
                  <div>
                    ...
                    <span className="text-[8px] font-black text-blue-600 uppercase">{p.days} Dias</span>
                  </div>
          ...
          <div className="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-sm min-h-[400px]">
            <div className="flex justify-between items-center mb-10">
              <div className="flex items-center">
                ...
                <div><h3 className="text-xl font-black text-slate-800 uppercase tracking-tighter">Operadores do Sistema</h3><p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Controle de acessos e logins</p></div>
              </div>
              <button onClick={() => setShowUserModal(true)} className="px-6 py-3 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl hover:bg-indigo-700 transition-all flex items-center"><UserPlus size={16} className="mr-2"/> Novo Operador</button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {(users || []).map((u) => (
                <div key={u.id} className="p-6 rounded-[2rem] border border-slate-100 bg-slate-50 flex items-center justify-between group hover:border-indigo-200 transition-all">
                  <div className="flex items-center space-x-4">
                    <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-indigo-600 shadow-sm border border-slate-100"><KeyRound size={24} /></div>
                    <div>
                      ...
                      <span className={`px-2 py-0.5 rounded-full text-[8px] font-black uppercase border mt-2 inline-block ${u.role === UserRole.ADMIN ? 'bg-indigo-600 text-white border-indigo-500' : 'bg-white text-slate-500 border-slate-200'}`}>{u.role}</span>
                    </div>
                  </div>
                  {u.username !== 'admin' && (
        ...
        <div className="space-y-6 animate-in fade-in">
          <div className="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-sm">
            <div className="flex items-center mb-10">
              ...
              <div>
                <h3 className="text-xl font-black text-slate-800 uppercase tracking-tighter">Canais de Notificação</h3>
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Integração EmailJS para disparos de alertas</p>
              </div>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              <div className="space-y-6">
                <h4 className="text-[11px] font-black text-slate-800 uppercase tracking-[0.2em] flex items-center border-b border-slate-100 pb-2"><Terminal size={14} className="mr-2 text-indigo-600"/> Credenciais</h4>
                <div className="space-y-5">
                  <div>
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Service ID</label>
                    <input value={safeEmailConfig.serviceId} onChange={e => setEmailConfig({...safeEmailConfig, serviceId: e.target.value})} className="w-full mt-2 border border-slate-200 rounded-2xl p-4 text-xs font-bold bg-slate-50 outline-none focus:ring-4 focus:ring-indigo-500/10" placeholder="Ex: service_rjs9xxx" />
                  </div>
                  <div>
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Template ID</label>
                    <input value={safeEmailConfig.templateId} onChange={e => setEmailConfig({...safeEmailConfig, templateId: e.target.value})} className="w-full mt-2 border border-slate-200 rounded-2xl p-4 text-xs font-bold bg-slate-50 outline-none focus:ring-4 focus:ring-indigo-500/10" placeholder="Ex: template_u8mpxxx" />
                  </div>
                  <div>
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Public Key</label>
                    <input value={safeEmailConfig.publicKey} onChange={e => setEmailConfig({...safeEmailConfig, publicKey: e.target.value})} className="w-full mt-2 border border-slate-200 rounded-2xl p-4 text-xs font-bold bg-slate-50 outline-none focus:ring-4 focus:ring-indigo-500/10" placeholder="Ex: user_0k2pxxx" />
                  </div>
                </div>
              </div>
              <div className="space-y-6">
                <h4 className="text-[11px] font-black text-slate-800 uppercase tracking-[0.2em] flex items-center border-b border-slate-100 pb-2"><Mail size={14} className="mr-2 text-indigo-600"/> Modelo</h4>
                <div className="space-y-5">
                  <div>
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Assunto</label>
                    <input value={safeEmailConfig.subject} onChange={e => setEmailConfig({...safeEmailConfig, subject: e.target.value})} className="w-full mt-2 border border-slate-200 rounded-2xl p-4 text-xs font-bold bg-slate-50 outline-none" />
                  </div>
                  <div>
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Mensagem</label>
                    <textarea value={safeEmailConfig.template} onChange={e => setEmailConfig({...safeEmailConfig, template: e.target.value})} className="w-full mt-2 border border-slate-200 rounded-2xl p-4 text-xs font-bold bg-slate-50 outline-none h-40 resize-none" />
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[2000] flex items-center justify-center p-4">
          <div className="bg-white rounded-[3rem] max-w-sm w-full p-10 shadow-2xl relative animate-in zoom-in duration-200 border border-slate-100">
            ...
            <form onSubmit={handleSaveParam} className="space-y-4">
              ...
              <input type="number" value={newParam.days} onChange={e => setNewParam({...newParam, days: parseInt(e.target.value)})} required placeholder="Dias" className="w-full border border-slate-200 rounded-2xl p-4 text-sm font-bold bg-slate-50 outline-none" />
              <button type="submit" className="w-full py-4 bg-blue-600 text-white font-black text-[10px] uppercase rounded-2xl shadow-xl mt-4 active:scale-95">Salvar</button>
            </form>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[2000] flex items-center justify-center p-4">
          <div className="bg-white rounded-[3rem] max-w-sm w-full p-10 shadow-2xl relative animate-in zoom-in duration-200 border border-slate-100">
            ...
            <form onSubmit={handleSaveUser} className="space-y-4">
              ...
              <input value={newUser.username} onChange={e => setNewUser({...newUser, username: e.target.value})} required placeholder="Login" className="w-full border border-slate-200 rounded-2xl p-4 text-sm font-bold bg-slate-50 outline-none" />
              <input type="password" value={newUser.password} onChange={e => setNewUser({...newUser, password: e.target.value})} required placeholder="Senha" className="w-full border border-slate-200 rounded-2xl p-4 text-sm font-bold bg-slate-50 outline-none" />
              ...
              <button type="submit" className="w-full py-4 bg-indigo-600 text-white font-black text-[10px] uppercase rounded-2xl shadow-xl mt-4 active:scale-95">Cadastrar</button>
            </form>
          </div>
        </div>
      )}
