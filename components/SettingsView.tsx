...
// Fix: All Lucide icons should be imported from 'lucide-react', not '../utils'
const { 
  Plus: PlusIcon, Trash2: TrashIcon, Building2: BuildingIcon, MapPin: MapIcon, X: XIcon, UserPlus: UserPlusIcon, Mail: MailIcon, 
  Clock: ClockIcon, Briefcase: BriefcaseIcon, Activity: ActivityIcon, Users: UsersIcon, Save: SaveIcon, ShieldAlert: ShieldIcon, 
  Lock: LockIcon, Info: InfoIcon, EyeOff: EyeOffIcon, Eye: EyeIcon, Scale: ScaleIcon, DatabaseZap: DatabaseZapIcon, 
  Bomb: BombIcon, RefreshCw: RefreshIcon, Check: CheckIcon, KeyRound: KeyIcon, Terminal: TerminalIcon
} = LucideIcons;
...
const ListManager: React.FC<ListManagerProps> = ({ title, subtitle, items, onAdd, onAction, onToggleStatus, icon }) => {
  ...
  const handleAdd = () => {
    if (inputValue.trim()) {
      // Fix: Use imported removeAccents from utils
        ...
        {items.map((item) => (
          <div key={item.id} className={`flex items-center justify-between p-3 rounded-xl border transition-all ${item.status === 'inactive' ? 'bg-slate-50 opacity-60 border-transparent' : 'bg-white border-slate-100'}`}>
            ...
            <div className="flex items-center">
              ...
              >
                {item.status === 'inactive' ? <EyeOffIcon size={14}/> : <EyeIcon size={14}/>}
              </button>
              <button 
...
COMMENT ON TABLE public.sistemp_data IS 'Armazenamento centralizado v2.4';`;
  ...
  const handleSaveUser = (e: React.FormEvent) => {
    e.preventDefault();
    const user: User = { ...newUser, id: Math.random().toString(36).substr(2, 9) };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      {activeSubTab === 'cloud' && (
        <div className="space-y-6 animate-in fade-in">
            <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm">
                <h3 className="text-sm font-black text-slate-800 uppercase tracking-tighter flex items-center mb-6"><ActivityIcon className="mr-2 text-indigo-600" size={18}/> Estado da Nuvem</h3>
                {cloudStatus === 'connected' ? (
                   <div className="p-8 bg-green-50 rounded-[2rem] border border-green-200 flex items-center justify-between">
                      <div className="flex items-center space-x-4">
                        <div className="w-12 h-12 bg-green-500 text-white rounded-2xl flex items-center justify-center shadow-lg"><CheckIcon size={24}/></div>
                        <div>
                  ...
                  <div className="p-8 bg-amber-50 rounded-[2rem] border border-amber-200 flex items-center justify-between animate-pulse">
                      <div className="flex items-center space-x-4">
                        <div className="w-12 h-12 bg-amber-500 text-white rounded-2xl flex items-center justify-center shadow-lg"><RefreshIcon size={24} className="animate-spin" /></div>
                        <div>
            ...
            <div className="bg-white p-10 rounded-[2.5rem] border-2 border-indigo-100 shadow-sm relative overflow-hidden">
                <div className="flex items-center space-x-3 mb-4">
                    <DatabaseZapIcon className="text-indigo-600" size={28}/><h3 className="text-xl font-black text-slate-800 uppercase tracking-tighter">Script SQL de Reparo Supabase</h3>
                </div>
                ...
                <p className="mt-4 text-[9px] font-bold text-slate-400 uppercase tracking-widest flex items-center"><InfoIcon size={12} className="mr-2"/> Utilize este script caso ocorra erro 42P01 ou perda de tabelas.</p>
            </div>

            <div className="bg-red-50 p-8 rounded-[2.5rem] border-2 border-red-200 shadow-sm mt-8">
                <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                        <div className="p-4 bg-red-600 text-white rounded-2xl shadow-lg"><BombIcon size={24}/></div>
                        <div>
                    ...
                    >
                        <RefreshIcon size={14} className="mr-2" /> Zerar Todo o Sistema
                    </button>
                </div>
            </div>
        </div>
        ...
        <div className="space-y-8 animate-in fade-in">
          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm">
            <div className="flex justify-between items-center mb-6">
              <div className="flex items-center">
                <div className="p-2.5 bg-blue-50 text-blue-600 rounded-xl mr-3"><ScaleIcon size={20}/></div>
              ...
              <button onClick={() => setShowParamModal(true)} className="px-5 py-2.5 bg-blue-600 text-white rounded-xl text-[9px] font-black uppercase tracking-widest shadow-md hover:bg-blue-700 transition-all flex items-center"><PlusIcon size={14} className="mr-1.5"/> Novo Prazo</button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {parameters.filter(p => p && p.label).map((p) => (
                <div key={p.id} className="p-4 rounded-2xl border border-slate-100 bg-white hover:border-blue-200 transition-all flex items-center justify-between group">
                  ...
                  <button onClick={() => setParameters(prev => prev.filter(item => item.id !== p.id))} className="p-2 text-slate-300 hover:text-red-500 transition-all opacity-0 group-hover:opacity-100"><TrashIcon size={14}/></button>
                </div>
              ))}
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <ListManager title="Órgãos" subtitle="Órgãos Solicitantes" items={agencies} onAdd={(name) => setAgencies((p:any) => [{id: Math.random().toString(36).substr(2, 9), name, status:'active'}, ...p])} onAction={(item) => setAgencies(prev => prev.filter(i => i.id !== item.id))} onToggleStatus={(item) => setAgencies(prev => prev.map(i => i.id === item.id ? {...i, status: i.status === 'active' ? 'inactive' : 'active'} : i))} icon={<BuildingIcon/>} />
            <ListManager title="Unidades" subtitle="Unidades de Lotação" items={units} onAdd={(name) => setUnits((p:any) => [{id: Math.random().toString(36).substr(2, 9), name, status:'active'}, ...p])} onAction={(item) => setUnits(prev => prev.filter(i => i.id !== item.id))} onToggleStatus={(item) => setUnits(prev => prev.map(i => i.id === item.id ? {...i, status: i.status === 'active' ? 'inactive' : 'active'} : i))} icon={<MapIcon/>} />
            <ListManager title="Perfis" subtitle="Perfis Profissionais" items={profiles} onAdd={(name) => setProfiles((p:any) => [{id: Math.random().toString(36).substr(2, 9), name, status:'active'}, ...p])} onAction={(item) => setProfiles(prev => prev.filter(i => i.id !== item.id))} onToggleStatus={(item) => setProfiles(prev => prev.map(i => i.id === item.id ? {...i, status: i.status === 'active' ? 'inactive' : 'active'} : i))} icon={<BriefcaseIcon/>} />
          </div>
        </div>
      )}
        ...
        <div className="space-y-6 animate-in fade-in">
          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm">
            <div className="flex justify-between items-center mb-8">
              ...
              <button onClick={() => setShowUserModal(true)} className="px-5 py-2.5 bg-indigo-600 text-white rounded-xl text-[9px] font-black uppercase tracking-widest shadow-md hover:bg-indigo-700 transition-all flex items-center"><UserPlusIcon size={14} className="mr-1.5"/> Novo Operador</button>
            </div>
        ...
        <div className="space-y-6 animate-in fade-in">
          <div className="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-sm">
            <div className="flex items-center mb-10">
              <div className="p-3 bg-indigo-50 text-indigo-600 rounded-2xl mr-4"><MailIcon size={24}/></div>
              <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              <div className="space-y-6">
                <h4 className="text-[11px] font-black text-slate-800 uppercase tracking-[0.2em] flex items-center"><TerminalIcon size={14} className="mr-2 text-indigo-600"/> Credenciais de Serviço</h4>
              ...
              <div className="space-y-6">
                <h4 className="text-[11px] font-black text-slate-800 uppercase tracking-[0.2em] flex items-center"><MailIcon size={14} className="mr-2 text-indigo-600"/> Modelo da Mensagem</h4>
                <div className="space-y-4">
                  <div>
                  ...
                  <div className="p-4 bg-indigo-50 rounded-2xl border border-indigo-100 flex items-center space-x-3">
                    <InfoIcon size={16} className="text-indigo-600 shrink-0" />
      ...
      {showParamModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[2000] flex items-center justify-center p-4">
          <div className="bg-white rounded-[3rem] max-w-sm w-full p-10 shadow-2xl relative animate-in zoom-in duration-200 border border-slate-100">
            <button onClick={() => setShowParamModal(false)} className="absolute top-8 right-8 text-slate-400 hover:text-slate-600"><XIcon size={20}/></button>
      ...
      {showUserModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[2000] flex items-center justify-center p-4">
          <div className="bg-white rounded-[3rem] max-w-sm w-full p-10 shadow-2xl relative animate-in zoom-in duration-200 border border-slate-100">
            <button onClick={() => setShowUserModal(false)} className="absolute top-8 right-8 text-slate-400 hover:text-slate-600"><XIcon size={20}/></button>
