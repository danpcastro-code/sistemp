...
const ConvocationManagement: React.FC<ConvocationManagementProps> = ({ 
  ...
  // Garantia absoluta de que pssList é tratado como array
  ...
  return (
    <div className="space-y-6">
      {/* CABEÇALHO COM BOTÃO SEMPRE VISÍVEL */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-5 rounded-[2.5rem] border border-slate-200 shadow-sm">
        <div className="flex space-x-2 bg-slate-100 p-1.5 rounded-2xl w-fit border border-slate-200">
          <button onClick={() => setActiveSubTab('classified')} className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase transition-all ${activeSubTab === 'classified' ? 'bg-white text-blue-600 shadow-md' : 'text-slate-500 hover:bg-slate-200'}`}>Classificação</button>
          <button onClick={() => setActiveSubTab('convoked')} className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase transition-all ${activeSubTab === 'convoked' ? 'bg-white text-blue-600 shadow-md' : 'text-slate-500 hover:bg-slate-200'}`}>Nomeados</button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 animate-in fade-in duration-300">
          ...
          <div className="lg:col-span-1">
              <div className="bg-white p-6 rounded-[2.5rem] border border-slate-200 shadow-sm h-fit min-h-[400px]">
                  ...
                  <div className="space-y-2 max-h-[500px] overflow-y-auto pr-1 custom-scrollbar">
                      ...
                      {filteredPssList.length === 0 && (
                          <div className="py-20 text-center opacity-30">
                              <p className="text-[9px] font-black uppercase tracking-widest">Nenhum edital ativo</p>
                          </div>
                      )}
                  </div>
              </div>
              ...
              {selectedPssId ? (
                <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden min-h-[500px]">
                  <div className="p-20 text-center opacity-40">
                    ...
                    <p className="text-[10px] font-black uppercase tracking-widest">Utilize as abas acima para gerenciar os candidatos.</p>
                  </div>
                </div>
              ) : (
                <div className="bg-white rounded-[3rem] border-4 border-dashed border-slate-100 p-32 flex flex-col items-center justify-center text-center h-full">
                    ...
                    <h3 className="text-2xl font-black text-slate-800 uppercase tracking-tighter">Central de Chamamentos</h3>
                    <p className="text-[10px] text-slate-400 font-bold uppercase mt-2 tracking-widest max-w-xs leading-relaxed">
                      Selecione um edital à esquerda ou crie um novo para gerenciar as classificações e nomeações.
                    </p>
                    {canEdit && (
      ...
      {showAddPssModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[2000] flex items-center justify-center p-4">
          <div className="bg-white rounded-[3rem] max-w-sm w-full p-10 shadow-2xl relative border border-slate-100 animate-in zoom-in duration-200">
            ...
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-8">Processo Seletivo Temporário</p>
            <form onSubmit={handleAddPss} className="space-y-4">
              <input value={newPssTitle} onChange={e => setNewPssTitle(e.target.value)} required placeholder="Ex: PSS 01/2024 - Saúde" className="w-full border border-slate-200 rounded-2xl p-4 text-sm font-bold bg-slate-50 outline-none focus:ring-4 focus:ring-blue-500/10 transition-all" />
              <div>
              ...
              <button type="submit" className="w-full py-4 bg-blue-600 text-white font-black text-[10px] uppercase rounded-2xl shadow-xl mt-4 active:scale-95 transition-all">Gravar Edital</button>
            </form>
          </div>
        </div>
      )}
