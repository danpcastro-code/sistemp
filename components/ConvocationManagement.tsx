...
import { Search, Plus, Edit, Clock, FileUp, X, CheckCircle2, AlertCircle, Users } from 'lucide-react';
    ...
    reader.onload = (event) => {
      try {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          if (competitionStr?.toLowerCase().includes('cotas') || competitionStr?.toLowerCase().includes('ppp')) competition = CompetitionType.PPP;
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div className="flex items-center space-x-3 w-full md:w-auto">
            ...
            <button 
                ...
                className="flex-1 md:flex-none px-6 py-3 bg-slate-100 text-slate-700 hover:bg-slate-200 rounded-2xl font-bold flex items-center justify-center transition-all border border-slate-200"
            >
                <FileUp size={18} className="mr-2 text-slate-500"/> Importar Planilha
            </button>
            <button 
                onClick={() => { setEditingPerson(null); setShowModal(true); }} 
                className="flex-1 md:flex-none px-8 py-3 bg-blue-600 text-white hover:bg-blue-700 rounded-2xl font-bold flex items-center justify-center shadow-lg shadow-blue-500/20 active:scale-95 transition-all"
            >
