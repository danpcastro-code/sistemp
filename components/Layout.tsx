...
import { LayoutDashboard, Users, FileText, Settings, History, Menu, X, Award, LogOut, ShieldCheck, CloudOff, RefreshCw, Zap, Wifi } from 'lucide-react';
...
const Layout: React.FC<LayoutProps> = ({ 
  ...
  const menuItems = [
    ...
    { id: 'convocations', label: 'Convocados', icon: Award, roles: [UserRole.ADMIN, UserRole.HR] },
          ...
          <div className="flex items-center space-x-6">
            ...
            <div className="flex items-center pl-6 border-l border-slate-100">
               {cloudStatus === 'connected' ? (
                 <div className="flex items-center text-green-600 bg-green-50 px-4 py-1.5 rounded-full border-2 border-green-200 shadow-sm animate-in fade-in group relative cursor-help">
                    <div className="pulse-ring mr-2 flex items-center justify-center">
                        <Zap size={14} className="text-green-500 fill-green-500 relative z-10" />
                    </div>
                    <span className="text-[10px] font-black uppercase tracking-widest flex items-center">
                        Ao Vivo 
                        <Wifi size={10} className="ml-1.5 opacity-50" />
                    </span>
                    <div className="absolute top-full mt-2 left-0 bg-slate-900 text-white text-[9px] font-bold p-3 rounded-xl w-48 hidden group-hover:block shadow-2xl z-50 leading-relaxed border border-slate-800">
                        Sincronização instantânea ativa. Alterações em outros dispositivos aparecerão aqui sem recarregar.
                    </div>
                 </div>
               ) : cloudStatus === 'syncing' ? (
                 <div className="flex items-center text-blue-600 bg-blue-50 px-4 py-1.5 rounded-full border border-blue-100">
               ...
               {cloudStatus !== 'idle' && (
                 <button onClick={onSync} className="ml-3 p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-all border border-transparent hover:border-blue-100">
                   <RefreshCw size={14} />
                 </button>
               )}
            </div>
